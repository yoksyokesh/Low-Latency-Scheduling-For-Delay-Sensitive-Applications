...
public class priority1 {
	...
	static int rand = 0;
	static double val = 0;
	static List<Integer> prio = new ArrayList<Integer>();
	static List<Integer> temp = new ArrayList<Integer>();
	static List<Integer> delay1 = new ArrayList<Integer>();
	static List<Integer> delay2 = new ArrayList<Integer>();
...
*/				moduleMapping.addModuleToDevice("connector", "cloud"); // fixing all instances of the Connector module to the Cloud
			...
			pre.addAll(tfog);
			for(int i=0; i<numOfDepts*numOfMobilesPerDept; i++) {
				delay2.add(-1);
			}
			int k=0;
			for(int i=1; i<=10; i++) {
				for(int j=0; j<numOfDepts*numOfMobilesPerDept; j++) {
					if(prio.get(j)==i) {
						delay2.remove(j);
						delay2.add(j, delay1.get(k));
						k++;
					}
